...
ETHICS_landMinesDoctrines = true;     // true = landmines will spawn if the minefield request them / false = turn it off. Default: true
	ETHICS_landmineTypeAP = "APERSMine";  // Default: "APERSMine" / https://community.bistudio.com/wiki/Arma_3:_CfgVehicles_Other (use the classname column's names)
	ETHICS_landmineTypeAM = "ATMine";     // Default: "ATMine" / https://community.bistudio.com/wiki/Arma_3:_CfgVehicles_Other (use the classname column's names)
ETHICS_navalMinesDoctrines = true;               // true = nauticalmines will spawn if the minefield request them / false = turn it off. Default: false
	ETHICS_navalmineTypeAM = "UnderwaterMineAB";  // Default: "UnderwaterMineAB"
...
[] spawn {
	...
	_txtWarning_3 = "ammunition is empty at fn_ETHICS_management.sqf file. To fix the error, the script set automatically an ammunition to be used.";
	// Handling errors:
	...
	if ( ETHICS_landMinesDoctrines AND (ETHICS_landmineTypeAP == "") ) then { ETHICS_landmineTypeAP = "APERSMine"; systemChat format ["%1 The AP %2", _txtWarningHeader, _txtWarning_3]};
	if ( ETHICS_landMinesDoctrines AND (ETHICS_landmineTypeAM == "") ) then { ETHICS_landmineTypeAM = "ATMine"; systemChat format ["%1 The AM %2", _txtWarningHeader, _txtWarning_3]};
	if ( ETHICS_navalMinesDoctrines AND (ETHICS_navalmineTypeAM == "") ) then { ETHICS_navalmineTypeAM = "UnderwaterMineAB"; systemChat format ["%1 The NAM %2", _txtWarningHeader, _txtWarning_3]};
	if ( !ETHICS_landMinesDoctrines AND !ETHICS_navalMinesDoctrines ) then { 
		systemChat format ["%1 There's no any minefield's doctrine available at fn_ETHICS_management.sqf file. Turn Landmines or Navalmines 'TRUE' to use Ethics Minefields script.", _txtWarningHeader];
	// If the basic is fine, the script starts:
		...
		{  // forEach ETHICS_confirmedMfMarkers (part 2/2):
			...
			{  // forEach _eachConfirmedList:
				...
				_mineAmountsByType = [_mfNameStructure, ETHICS_landmineTypeAP, ETHICS_landmineTypeAM, ETHICS_navalmineTypeAM, _x, _mfSize, _limiterMinefield, _mineAmountsByType] call THY_fnc_ETHICS_mine_planter;  // returns the mines' numbers updated.
			} forEach _eachConfirmedList;
		} forEach ETHICS_confirmedMfMarkers;
