...
ETH_doctrinesLandMinefield = true;     // true = landmines will spawn if an area-marker requests them / false = turn it off. Default: true
	...
	ETH_ammoLandAM = "ATMine";       // Default: "ATMine". For more device options, check the Ethics Documentation.
		ETH_onlyOnRoadsAM = true;      // true = AM landmines will be planted only on roads / false = turn it off. Default: false
...
ETH_globalDevicesIntensity = "MID";    // Proportional number of explosive devices through the area-markers. Options: "EXTREME", "HIGH", "MID", "LOW". Default: "MID"
...
[] spawn {
	...
	if ( ETH_doctrinesOXU AND ((count ETH_ammoPackUXO) == 0) ) then { ETH_ammoPackUXO = ["BombCluster_01_UXO2_F"]; systemChat format ["%1 The UXO %2", _txtWarningHeader, _txtWarning_3]};
